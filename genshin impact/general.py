...
class General(commands.Cog):   
  def embed_weapon(self, weapon):
    ...
    for i in range(weapon['rarity']):
      stars += ' ⭐'
    embed = discord.Embed(title=weapon['name'], description=stars, color=discord.Color.orange())
  ...
  def embed_artifact(self, artifact):
    stars = ''
    embed = discord.Embed(title=artifact['set'], description=stars, color=discord.Color.orange())
  ...
  async def exp(self, ctx):
    ...
    await ctx.send("https://media.discordapp.net/attachments/831356457554280452/863109049813565450/image_1.png?width=180&height=449")
