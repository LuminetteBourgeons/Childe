...
class Info(commands.Cog):
  ...
  async def userinfo(self, ctx, user: discord.Member = None):
    ...
    embed = discord.Embed(title=name + "'s Information<:Chchibi:843379361138737182>", color = orange)
  ...
  async def permissions(self, ctx, user: discord.Member = None):
    ...
    embed = discord.Embed(title=f'{user.name}' + "'s Permissions<:Chchibi:843379361138737182>", description=f'`{perms}`', color = orange)
    embed.set_thumbnail(url=avatar)
    embed.set_footer(text=f"Serving: {ctx.author.name}", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=embed)
  ...
  async def serverinfo(self, ctx):
    ...
    time = time.split(" ")
    time = time[0]
    embed = discord.Embed(title=name + " Server Information<:Chchibi:843379361138737182>", color = orange)
    embed.set_thumbnail(url=icon)
    embed.add_field(name="Server Owner", value=owner, inline=True)
    embed.add_field(name="Server ID", value=id, inline=True)
    embed.add_field(name="‎", value="‎", inline=True)
    embed.add_field(name="<:Chchibi:843379361138737182>Members Informations:", value=f"All members: `{total_member}`\nHumans (hopefully): `{humans}`\nBots: `{bots}`\nOnline members: `{online_members}`\nOffline members: `{offline_members}`", inline=True)
    embed.add_field(name="<:Chchibi:843379361138737182>Server Informations:", value=f"Total roles: `{roles}`\nCategories: `{categories}`\nTotal channels: `{channels}`\nText channels: `{text_channels}`\nVoice channels: `{voice_channels}`\nBoost level: `{boost_level}`\nTotal boost: `{total_boosts}`\nServer created at: {time}", inline=True)
    embed.set_footer(text=f"Serving: {ctx.author.name}", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=embed)
  ...
  async def botinfo(self, ctx, bot_user: discord.Member):
    ...
    try:
      ...
      em.add_field(name="Library", value=bot.get("libraryName"))
