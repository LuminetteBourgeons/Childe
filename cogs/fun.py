...
class Fun(commands.Cog):
  ...
  async def dice(self, ctx):
    ...
    embed = discord.Embed(title=f"<:Chchibi:843379361138737182> Rolled a dice! 🎲", description=f"It landed on {random.choice(dice6)}!",colour=discord.Color.orange())
  ...
  async def coin(self, ctx):
    ...
    await asyncio.sleep(3)
    embed = discord.Embed(title=f"<:Chchibi:843379361138737182> Flipped a coin! 🪙", description=f"It landed on {random.choice(coin2)}. {random.choice(childe)}",colour=discord.Color.orange())
    embed.set_footer(text=f"Serving: {ctx.author.name}", icon_url=ctx.author.avatar_url)
    await ctx.send(embed=embed)
